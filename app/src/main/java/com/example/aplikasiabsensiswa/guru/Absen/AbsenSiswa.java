...
public class AbsenSiswa extends AppCompatActivity {
    ...
    public String getCheckedSiswa() {
        List<String> checkedSiswa = new ArrayList<>();
        for (Siswa siswa : allSiswa) {
            if (siswa.isChecked().equals("masuk")) {
                checkedSiswa.add(siswa.getNama());
            }
        }
        return String.join(", ", checkedSiswa);
    ...
    public String getSiswaAlpha() {
        List<String> checkedSiswa = new ArrayList<>();
        for (Siswa siswa : allSiswa) {
            if (siswa.isChecked().equals("alpha")) {
                checkedSiswa.add(siswa.getNama());
            }
        }
        return String.join(", ", checkedSiswa);
    ...
    public String getSiswaSakit() {
        List<String> checkedSiswa = new ArrayList<>();
        for (Siswa siswa : allSiswa) {
            if (siswa.isChecked().equals("sakit")) {
                checkedSiswa.add(siswa.getNama());
            }
        }
        return String.join(", ", checkedSiswa);
    ...
    public String getSiswaIzin() {
        List<String> checkedSiswa = new ArrayList<>();
        for (Siswa siswa : allSiswa) {
            if (siswa.isChecked().equals("izin")) {
                checkedSiswa.add(siswa.getNama());
            }
        }
        return String.join(", ", checkedSiswa);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String nip = sharedPreferences.getString("nomorInduk","");

        button.setOnClickListener(v -> {
            Absen absen = new Absen(Integer.parseInt(textJamKe.getText().toString()), getIntent().getStringExtra("kelasKey"), nip, Integer.parseInt(sampaiJamKe.getText().toString()), date, getAllSiswa(), getCheckedSiswa(), getSiswaAlpha(), getSiswaIzin(), getSiswaSakit());
